...
uint256 hashGenesisBlock("0x0000003f56e4c8a371f8b00edb823003ffca6aa434292ba1223f3389b0953b61");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BitfxCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BitfxCoin Signed Message:\n";
...
{
    // BitfxCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Launch specs
static const int64 nTargetTimespan = 1 * 60; // BitfxCoin: each block
static const int64 nTargetSpacing = 1 * 60; // BitfxCoin: 60 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BitfxCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, BitfxCoin - DarkGravity, written by Evan Duffield - evan@BitfxCoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BitfxCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // BitfxCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        assert(block.hashMerkleRoot == uint256("0x88daa45d42980c3b388505216f1d75c550b07bedc2ca038333e37d5edef01319"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa6, 0xd2, 0xc3, 0xf6 }; // BitfxCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitfxCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitfxCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitfxCoinMiner(CWallet *pwallet)
{
    printf("BitfxCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("BitfxCoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running BitfxCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitfxCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitfxCoinMiner, pwallet));
}
